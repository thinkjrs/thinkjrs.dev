...
Authenticating with Google Cloud is pretty easy when you're rocking a notebook or using a Virtual Machine. 

But if you develop production software for the web, getting authenticated can be more of a challenge. For reasons I won't go into here, loading a file from disk is typically not an option in production web environments and very, very unsafe from a practicality standpoint. 
...
In the code above we us the `os` module to load the environment variable and send its contents to the `json.loads` method, which turns the string JSON into a dictionary, containing our service account values.
