...
serve client users. You can read about that on our [Slightly Sharpe blog](https://slightlysharpe.com/data-modeling-at-tincre) for the deets.

But here, I'm simply going to show you how we actually merge Python dictionaries
that have lots of `Nonetype` values and sparsely, string or other values.
...
If you're a regular Pythonista you know that this behavior isn't totally built-in or rather, off-the-shelf.
...
Yeap, that's super cool and the community was super happy with the addition to the Python `dict` build-in. Life is easier.

And yes, for those language lawyers out there, you can also do the same thing in-place:

```python
my_dict0 = dict(awesomeKeyName = "awesome value 0")
my_dict1 = dict(awesomeKeyName = "awesome value 1", anotherAwesomeKeyName = "awesome value 1 for anotherAwesomeKeyName")
...
What happesn when one of these key values is of type `None`? In particular, will `Nonetype` values overwrite
...
```python
my_dict0 = dict(awesomeKeyName = "awesome value 0")
my_dict1 = dict(awesomeKeyName = None, anotherAwesomeKeyName = "awesome value 1 for anotherAwesomeKeyName")
# in-place modification of my_dict0
my_dict0 |= my_dict1
print(my_dict0)
```
...
Perfect. Empty strings, boolean values, and everything else that isn't `Nonetype` is untouched.
...
def merge_without_none(lhs: dict, rhs: dict) -> dict:
    """Merge lhs and rhs dictionaries, exluding from both and 
...
assert "test val" in merge_without_none({"testKey": "test val"}, {"testKey": None}).get("testKey")
assert not merge_without_none({"testKey": None}, {"testKey": None}).get("testKey")
```
...
of those classes instantiated.

#### Instantiate our `PaymentFeatures` objects
```python
# instantiate the objects
pf0 = PaymentFeatures(myParam=42, myOtherParam="Is the meaning of life",)
pf1 = PaymentFeatures(myParam=42, myOtherParam=None,)
```
...
```python
pf_merged = merge_without_none(pf0.dict(), pf1.dict())
```
