...
Because we implement the common trait `Anything` that makes sure our diverse types like `Star` and `Mansion` provide a `describe` method, so they can behave uniformly.
